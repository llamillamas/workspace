#!/usr/bin/env python3
"""
Option B Collection v3: CORRECTED
- Dynamic market ID calculation (not hardcoded)
- Sliding window tracking (current ± 6 markets)
- Order books from ACTIVE markets only
- Proper error logging for diagnosis
"""
...
PRICE_LOG = DATA_DIR / "prices_v3.jsonl"
ORDERBOOK_LOG = DATA_DIR / "orderbooks_v3.jsonl"
RESOLUTION_LOG = DATA_DIR / "resolutions_v3.csv"
LOG_FILE = DATA_DIR / "option_b_v3_log.txt"
...
log("=" * 100)
log("🚀 OPTION B COLLECTION V3 STARTED (CORRECTED)")
log("=" * 100)
        ...
        if elapsed - last_market_discovery >= market_discovery_interval:
            ...
            for offset in [-900, -600, -300, 0, 300, 600]:
                ...
                try:
                    ...
                    log(f"⚠️ Market discovery error for {slug}: {e}")
        
        # === CHECK FOR RESOLUTIONS (every 10 seconds) ===
        if elapsed - last_market_discovery >= 10:
            for slug, market_data in list(tracked_markets.items()):
                ...
                try:
                    ...
                    if markets:
                        ...
                        tracked_markets[slug]['closed'] = is_closed
                        
                        # If just became closed, record resolution
                        if is_closed and not market_data['closed']:
                            ...
                            try:
                                ...
                                log(f"⚠️ Resolution recording error: {e}")
                
                except Exception as e:
                    log(f"⚠️ Resolution check error: {e}")
        
        # === GET ORDER BOOKS FROM ACTIVE MARKETS (every 10 seconds) ===
        if elapsed - last_ob_check >= ob_interval:
            last_ob_check = elapsed
            
            # Get order books ONLY from active markets
            active_count = 0
            for slug, market_data in list(tracked_markets.items()):
                if not market_data['active'] or market_data['closed']:
                    continue
                ...
                active_count += 1
                if active_count > 3:  # Limit to 3 active markets per check
                    break
                ...
                try:
                    ...
                    log(f"⚠️ Order book error for {slug}: {e}")
